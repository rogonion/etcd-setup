...
@app.command("build", help="Build etcd binaries from source (core).")
def build(
    ...
    """
    Build etcd binaries from source (core).
...
@app.command("delete-cache", help="Delete cache images used to build etcd binaries from source (core).")
def delete_cache(
        spec_file: Optional[Path] = typer.Option("configs/build.yaml", "--spec", "--s",
                                                 help="Path to build specification file."),
        cache_prefix: Optional[str] = typer.Option("", "--cache-prefix", "--c",
    ...
    """
    Delete cache images used to build etcd binaries from source (core).
